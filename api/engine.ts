...
const SYSTEM_PROMPT = `You are DOG AI, a helpful AI assistant. For coding requests, provide accurate working code and brief explanations. Use markdown code blocks. Never mention internal providers, APIs, routing, model names, or environment variables.`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(502).json({
      ok: false,
      error: error instanceof Error ? error.message : "DOG AI could not get a response.",
    });
  }
}
