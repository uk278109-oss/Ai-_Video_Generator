...
import { Menu, Image as ImageIcon, Code2, Sparkles, Brain, UserCircle2 } from "lucide-react";
...
export default function Home({ onOpenMenu, onNavigate, onOpenAccount }: HomeProps) {
  ...
  return <div className="home">
    <header className="mobile-header">
      ...
      <div className="mobile-brand">DOG AI</div>
    ...
    {messages.length > 0 && <section className="conversation-preview">
      {messages.map((m, i) => <div className={`message-bubble ${m.role}`} key={i}><div className="preview-label">{m.role === "user" ? "You" : `DOG AI${m.provider ? ` • ${m.provider}` : ""}`}</div><div className="preview-message">{m.content}</div></div>)}
      {loading && <div className="ai-loading"><DogLoader size={34} label="DOG AI is thinking…" /></div>}
    </section>}
    ...
    <section className="hero-section">
      ...
      <p>Ask DOG AI for code, debugging help, architecture or ideas.</p>
    ...
    <section className="chat-section"><ChatInput onSend={handleSend} disabled={loading}/><div className="bottom-placeholder"><span>DOG AI • Coding + Images</span></div></section>
  </div>;
}
